...
* **AI Model Engine (Gemini Enterprise):** Powered by Google Gemini 3.7 Flash (`google-genai 1.75.0` / `2.0+`) for multimodal reasoning, scene pacing, and diffusion visual directives.
* **Brand Governance Gate (IBM watsonx):** Primary hackathon track spine. The governance adapter enforces customizable policy packs (*General Audience*, *Kids & Family*, *Mature Documentary*). In deployed instances with `IBM_WATSONX_API_KEY` configured, it calls the live evaluation endpoint; when running offline or in sandbox mode, it executes deterministic keyword and risk heuristics.
